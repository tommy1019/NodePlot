...
ErrorOr<nlohmann::json> NodeGraph::storage_to_json(NodeStorage node_storage) {
...
ErrorOr<NodeGraph::NodeStorage> NodeGraph::storage_from_json(nlohmann::json json, NodeId node_id) {
    ...
    node_storage.type_id = TRY(get_typed(std::type_identity<NodeTypeId>{}, json, "type_id", "Missing NodeTypeId for node " + std::to_string(node_id)));

    auto pos_json = TRY(get_typed(std::type_identity<nlohmann::json>{}, json, "pos", "Missing node position for node " + std::to_string(node_id)));
    ...
    auto inputs_json = TRY(get(json, "inputs", "Missing Node Inputs for node " + std::to_string(node_id)));
...
ErrorOr<nlohmann::json> NodeGraph::to_json() {
    ...
    for (auto [node_id, node_storage] : nodes) {
        nodes_json[std::to_string(node_id)] = TRY(storage_to_json(node_storage));
    }
