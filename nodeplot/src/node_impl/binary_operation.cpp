...
template <>
ErrorOr<std::map<OutputIndex, NodeOutput>> node_output(EvaluatedNodeGraph* graph, GlobalNodeId id, const BinaryOperationNode& node) {

    auto a_val = TRY_OR(graph->get_output({.id = node.i_a.node, .graph_name = id.graph_name}, node.i_a.output_index), return ERR("Failed to get value for 'A'"));
    auto b_val = TRY_OR(graph->get_output({.id = node.i_b.node, .graph_name = id.graph_name}, node.i_b.output_index), return ERR("Failed to get value for 'B'"));
    auto operation = TRY_OR(graph->get_input(id.graph_name, node.i_operation), return ERR("Could not get 'Operation' input"));
    ...
    auto res = TRY(std::visit(overloaded{
    ...
    return std::map<OutputIndex, NodeOutput>{{"result", res}};
}
